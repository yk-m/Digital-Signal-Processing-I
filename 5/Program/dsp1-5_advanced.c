...
void hanningWindow( complex_t xn[], complex_t xn_windowed[], int N );
void blackman( complex_t xn[], complex_t xn_windowed[], int N );
...
int main()
{
	...
	printComplexToFile( "windowedData_hamming.txt",  xn_windowed, dim );
	...
	printDoubleToFile( "windowedAmpSpectrum_hamming.txt",  ampSpectrum, dim );

	hanningWindow( xn, xn_windowed, dim );
	printComplexToFile( "windowedData_hanning.txt",  xn_windowed, dim );
	...
	printDoubleToFile( "windowedAmpSpectrum_hanning.txt",  ampSpectrum, dim );

	blackman( xn, xn_windowed, dim );
	printComplexToFile( "windowedData_blackman.txt",  xn_windowed, dim );
	...
	printDoubleToFile( "windowedAmpSpectrum_blackman.txt",  ampSpectrum, dim );
...
void hanningWindow( complex_t xn[], complex_t xn_windowed[], int N ) {
	for (int n = 0; n < N; ++n)
	{
		xn_windowed[ n ].re = xn[n].re * ( 0.5 - 0.5*cos( 2*M_PI*n / N ) );
	}
}
...
void blackman( complex_t xn[], complex_t xn_windowed[], int N ) {
	for (int n = 0; n < N; ++n)
	{
		xn_windowed[ n ].re = xn[n].re * ( 0.42 - 0.5*cos( 2*M_PI*n / N ) + 0.08*cos( 4*M_PI*n / N ) );
	}
}
