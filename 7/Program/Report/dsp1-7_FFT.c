...
{
	int dim;
	complex_t *xn, *xn_sorted, *twiddles;
	...
	for ( int i = 0; i < 100; i++ ) {
		...
		fft( xn_sorted, twiddles, dim );
		t_end = clock();
		fft_time += (double)(t_end - t_start)/(double)CLOCKS_PER_SEC;

		printf("%d回目：%f ", i+1, (double)(t_end - t_start)/(double)CLOCKS_PER_SEC);

		t_start =  clock();
		conjugateComplexes( twiddles, dim );
		fft( xn_sorted, twiddles, dim);
		t_end = clock();
		ifft_time += (double)(t_end - t_start)/(double)CLOCKS_PER_SEC;
